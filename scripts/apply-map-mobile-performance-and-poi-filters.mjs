...
const testSource = [
  ...
  'test("mobile and desktop start with the vector map style", () => { assert.match(map, /style:\\s*(?:mapAssetProxyUrl\\(PRIMARY_MAP_STYLE\\)\\s*\\?\\?\\s*)?PRIMARY_MAP_STYLE/); assert.doesNotMatch(map, /mobile_raster_fast_path/); });',
