...
if (!mapSource.includes("style: PRIMARY_MAP_STYLE")) {
...
const testSource = [
  ...
  'test("mobile and desktop start with the vector map style", () => { assert.match(map, /style: PRIMARY_MAP_STYLE/); assert.doesNotMatch(map, /mobile_raster_fast_path/); });',
  'test("the raster style remains a bounded fallback", () => { assert.match(map, /map\\.setStyle\\(RASTER_FALLBACK_STYLE\\)/); });',
