...
export function computeRange(range: QuickRange, base: Date = new Date()): { from: Date; to: Date } {
  ...
  switch (range) {
    ...
    case "tonight": {
      ...
      const from = new Date(day); from.setHours(18, 0, 0, 0);
      const to = new Date(day); to.setDate(to.getDate() + 1); to.setHours(6, 0, 0, 0);
      ...
      if (b.getHours() < 6) {
        const yesterday = new Date(day); yesterday.setDate(yesterday.getDate() - 1); yesterday.setHours(18, 0, 0, 0);
    ...
    case "weekend": {
      ...
      const from = new Date(day); from.setDate(from.getDate() + daysToFri); from.setHours(18, 0, 0, 0);
      const to = new Date(from); to.setDate(to.getDate() + 2); to.setHours(23, 59, 0, 0);
      return { from, to };
    }
    case "week": {
...
export async function fetchCities() {
  const { data, error } = await supabase.from("cities").select("id,slug,name,timezone,latitude,longitude,is_demo").order("name");
  if (error) throw error;
  return data ?? [];
}
