    ...
    <div className="mx-auto max-w-6xl px-4 pt-6 md:px-6 md:pt-9">
      <section className="relative mb-6 overflow-hidden rounded-[2rem] border p-5 sm:p-7">
        ...
        <Badge className="mb-4 border-transparent bg-primary/15 text-primary">
          <Radio className="mr-1.5 h-3.5 w-3.5" /> {tr("Le réseau Global Party")}
        </Badge>
        <h1 className="max-w-3xl text-3xl font-black leading-tight sm:text-5xl">
          {tr("Les événements commencent bien avant l'ouverture des portes.")}
        </h1>
        ...
        <aside className="hidden space-y-4 lg:sticky lg:top-24 lg:block">
          {!userId && !currentUser.isLoading && (
            <div className="glass rounded-3xl p-5">
              ...
              <h2 className="mt-3 text-lg font-semibold">{tr("Rejoins la conversation")}</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                {tr("Connecte-toi pour aimer, commenter et échanger avec la communauté.")}
              </p>
              <Button asChild className="mt-4 w-full rounded-full">
                <a href="/auth?redirect=%2Fsocial">{tr("Se connecter")}</a>
              </Button>
            </div>
          )}

          <div className="glass rounded-3xl p-5">
            <h2 className="font-semibold">{tr("Ici, tu peux")}</h2>
            <ul className="mt-4 space-y-3 text-sm text-muted-foreground">
              <li className="flex gap-2.5">
                <BadgeCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{" "}
                {tr("Voir les annonces officielles")}
              </li>
              <li className="flex gap-2.5">
                <Heart className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{" "}
                {tr("Soutenir les publications")}
              </li>
              <li className="flex gap-2.5">
                <MessageCircle className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{" "}
                {tr("Échanger avec la communauté")}
              </li>
            </ul>
          </div>
          ...
          {userId && (
            <div className="rounded-3xl border border-primary/20 bg-primary/5 p-5">
              <p className="text-sm font-semibold">{tr("Tu organises des événements ?")}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {tr("Crée ou rejoins une organisation pour publier dans le fil.")}
              </p>
              <Button asChild variant="outline" size="sm" className="mt-3 rounded-full">
                <Link to="/organizer">{tr("Portail organisateur")}</Link>
              </Button>
            </div>
          )}
        </aside>
