...
export function EventCard({ ev }: { ev: DiscoveredEvent }) {
  ...
  useEffect(() => {
    if (!userId) return;
    supabase.from("favorites").select("event_id").eq("user_id", userId).eq("event_id", ev.event_id).maybeSingle()
      .then(({ data }) => setFav(!!data));
  }, [userId, ev.event_id]);

  const toggleFav = async (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    if (!userId) { toast.error("Connecte-toi pour enregistrer un événement"); return; }
    if (fav) {
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-muted">
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-3">
          <div className="flex flex-wrap gap-1.5">
            {ev.is_demo && (
              <Badge variant="outline" className="border-transparent" style={{ background: "var(--color-demo)", color: "var(--color-primary-foreground)" }}>
                Démonstration
              </Badge>
            )}
            {ev.is_free && <Badge style={{ background: "var(--color-secondary)", color: "var(--color-secondary-foreground)" }}>Gratuit</Badge>}
          ...
          >
            <Heart className="h-4 w-4" fill={fav ? "currentColor" : "none"} style={{ color: fav ? "var(--color-primary)" : "inherit" }} />
          </button>
        </div>
        {ev.category_slug && (
          <div className="absolute bottom-3 left-3">
            <Badge className="border-transparent" style={{ background: "oklch(0 0 0 / 0.55)", color: "white" }}>
              {ev.category_slug}
            </Badge>
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          {ev.is_verified && <BadgeCheck className="h-4 w-4 shrink-0" style={{ color: "var(--color-primary)" }} />}
        </div>
        ...
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span className="truncate">{ev.venue_name ?? "Lieu à confirmer"}{ev.city_name ? ` · ${ev.city_name}` : ""}</span>
          {ev.distance_km != null && <span className="ml-auto shrink-0 text-[10px]">{ev.distance_km} km</span>}
        </div>
      </div>
    </Link>
  );
