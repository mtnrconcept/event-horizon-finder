...
import { PlaceUpcomingEventsTab } from "@/components/place-upcoming-events-tab";
...
export function PlaceDetailDialog({
  ...
  const [activeTab, setActiveTab] = useState<"place" | "events">("place");
  ...
  const photos = useMemo(() => {
    ...
    return [
      ...new Set(
        [...(current.image_urls ?? []), current.image_url].filter(
  ...
  const sources = useMemo(() => {
    ...
    const fallback = (current.source_urls ?? []).map((url, index) => ({
      label: `${tr("Source")} ${index + 1}`,
      url,
      provider: null,
    }));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="map-event-dialog grid h-[min(94dvh,58rem)] max-w-5xl grid-rows-[auto_minmax(0,1fr)] gap-0 overflow-hidden rounded-3xl border-border bg-background p-0 shadow-2xl"
      >
        {current ? (
          <>
            <div
              className="relative h-[min(15rem,28dvh)] shrink-0 overflow-hidden"
              style={{
            ...
            >
              ...
              {detailLoading && (
                <div className="absolute right-14 top-4 rounded-full bg-black/60 p-2 text-white">
            ...
            <div className="min-h-0 overflow-y-auto overscroll-contain p-4 pt-3 sm:p-6 sm:pt-3">
              ...
              <div
                role="tablist"
                aria-label={tr("Contenu de la fiche du lieu")}
                className="mt-4 grid grid-cols-2 rounded-2xl border bg-muted/40 p-1"
              >
                <button
                  type="button"
                  role="tab"
                  ...
                  className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-3 text-sm font-black transition-colors"
                  style={
                    activeTab === "place"
                ...
                >
                  <Info className="h-4 w-4" /> {tr("Le lieu")}
                </button>
                <button
                  type="button"
                  role="tab"
                ...
                >
                  <CalendarDays className="h-4 w-4" /> {tr("Événements")}
                </button>
              </div>

              {activeTab === "events" ? (
                <div role="tabpanel" className="mt-5">
                  <PlaceUpcomingEventsTab placeId={current.id} />
                </div>
              ) : (
                <div role="tabpanel">
                  <p className="mt-4 whitespace-pre-line text-sm leading-6 text-muted-foreground sm:text-base">
                    {description}
                  </p>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {bookingUrl && (
                      <a
                        ...
                        className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-secondary px-4 py-2 text-sm font-black text-secondary-foreground"
                      >
                        <CalendarCheck2 className="h-4 w-4" /> {tr("Réserver")}{" "}
                        ...
                        <div className="no-scrollbar flex snap-x gap-3 overflow-x-auto pb-1">
                          {photos.map((photo, index) => (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="block w-52 shrink-0 snap-start overflow-hidden rounded-xl border"
                            >
                              <img
                                src={photo}
                    ...
                    <DetailSection icon={MapPin} title={tr("Adresse et localisation")}>
                      <div className="grid gap-2">
                        <p className="font-black">
                          {current.address ?? tr("Adresse non renseignée")}
                        </p>
                    ...
                    <DetailSection icon={Clock3} title={tr("Horaires et accès")}>
                      <dl className="grid gap-2">
                        <div>
                          ...
                          <dd className="mt-1">
                            {current.opening_hours ?? tr("À vérifier sur place")}
                          </dd>
                        </div>
                        <div>
                          <dt className="text-xs font-black uppercase text-muted-foreground">
                          ...
                          <dd className="mt-1">
                            {free ? tr("Gratuit") : (current.fee ?? tr("Non renseigné"))}
                          </dd>
                        </div>
                        <div>
                          <dt className="text-xs font-black uppercase text-muted-foreground">
                          ...
                          <dd className="mt-1">{current.wheelchair ?? tr("Non renseigné")}</dd>
                        </div>
                      </dl>
                    </DetailSection>
                    <DetailSection icon={Info} title={tr("Informations sur le lieu")}>
                      <dl className="grid gap-2">
                        <div>
                          ...
                          <dd className="mt-1">{placeCategoryLabel(current.category)}</dd>
                        </div>
                        {current.subcategory && (
                          <div>
                            <dt className="text-xs font-black uppercase text-muted-foreground">
                            ...
                            <dd className="mt-1">{current.subcategory.replaceAll("_", " ")}</dd>
                          </div>
                        )}
                      </dl>
                    </DetailSection>
                    <DetailSection icon={Globe2} title={tr("Origine des informations")}>
                      <div className="grid gap-2">
                        <p className="text-muted-foreground">
                          {tr(
                            "Informations regroupées depuis OpenStreetMap, des sites officiels, des guides touristiques et des sources publiques attribuées.",
                          )}
                        </p>
                        {sources.map((source) => {
                          const url = safeExternalUrl(source.url);
                        ...
                        {sourceUrl &&
                          !sources.some((source) => safeExternalUrl(source.url) === sourceUrl) && (
                            <a
                              href={sourceUrl}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 font-black text-primary hover:underline"
                            >
                              {tr("Voir la source cartographique")}{" "}
                              <ExternalLink className="h-3.5 w-3.5" />
                            </a>
                          )}
                      </div>
