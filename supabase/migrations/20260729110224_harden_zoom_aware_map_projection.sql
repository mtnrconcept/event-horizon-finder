-- Apply the hardened public projection model to the current zoom-aware RPC.
-- Explicit event and venue predicates preserve the public RLS contract while
-- owner execution lets PostGIS use the viewport GiST index.
...
comment on function public.discover_map_pins_in_bounds_v2(
  ...
  boolean, integer, integer, boolean, boolean, boolean, boolean, boolean,
  double precision
) is 'Hardened public zoom-aware map projection. Uses explicit public predicates and returns server clusters below zoom 8 or at most 12,000 individual pins.';
