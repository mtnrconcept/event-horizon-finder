-- The legacy endpoint is a deliberately narrow public projection. Running its
-- candidate scan as the function owner avoids the correlated occurrence RLS
-- check that prevents PostGIS from choosing the GiST viewport index. Every
-- exposed field remains guarded by the same public event and venue predicates.
...
begin
  ...
  execute $query$
    with indexed_occurrences as materialized (
      ...
      join public.events as public_event
        on public_event.id = occurrence.event_id
       and public_event.is_demo = false
       and public_event.status in ('published', 'cancelled', 'postponed', 'sold_out')
    ...
    unlocated_occurrences as materialized (
      select
        occurrence.id,
        occurrence.event_id,
        occurrence.capacity,
    ...
    fallback_points as materialized (
      ...
      from (
        ...
        from unlocated_occurrences as occurrence
    ...
    filtered_pins as materialized (
      ...
      order by point.starts_at, point.occurrence_id
      limit 4000
    )
    select coalesce(
      json_agg(pin.marker order by pin.starts_at, pin.occurrence_id),
      '[]'::json
    )
    from filtered_pins as pin
  $query$
  into result
  using
    _west,
...
comment on function public.discover_map_pins_in_bounds_v1(
  ...
  timestamptz, timestamptz, text[], boolean, text, text[], numeric, numeric,
  boolean, integer, integer, boolean, boolean, boolean, boolean, boolean
) is 'Hardened public compatibility projection for pre-v2 clients. Uses explicit public predicates, bounds candidate work to 7,000 rows, and returns at most 4,000 legacy pins.';
