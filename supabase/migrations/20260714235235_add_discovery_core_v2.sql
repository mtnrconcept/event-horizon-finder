-- Keep discovery responsive while the worldwide catalogue grows.
-- Ticket summaries are resolved through the event_id index only for candidate
-- rows instead of aggregating the complete ticket_offers table on every call.

CREATE OR REPLACE FUNCTION public.discover_event_rows_v2(
...
AS $$
  WITH event_points AS (
    ...
    WHERE event.status IN ('published','cancelled','postponed','sold_out')
      ...
      AND (
        _lat IS NULL OR _lon IS NULL
        OR coalesce(occurrence.location, venue.location, resolved_city.location) IS NULL
        OR public.st_dwithin(
          coalesce(occurrence.location, venue.location, resolved_city.location),
  ...
  WHERE (
    ...
    OR (point.latitude IS NOT NULL AND point.longitude IS NOT NULL)
  )
...
REVOKE ALL ON FUNCTION public.discover_event_rows_v2(
...
COMMENT ON FUNCTION public.discover_event_rows_v2(
  DOUBLE PRECISION, DOUBLE PRECISION, NUMERIC, TIMESTAMPTZ, TIMESTAMPTZ,
  TEXT[], UUID, BOOLEAN, TEXT, TEXT[], NUMERIC, NUMERIC, BOOLEAN,
  INT, INT, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, INT, INT
) IS 'Internal indexed discovery core. Ticket offers are summarized per candidate event.';
