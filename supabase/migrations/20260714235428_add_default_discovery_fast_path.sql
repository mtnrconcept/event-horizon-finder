-- Fast path for the overwhelmingly common catalogue request: a city and date
-- window without advanced filters or browser geolocation.

CREATE OR REPLACE FUNCTION public.discover_event_rows_default_v2(
  _from TIMESTAMPTZ,
  _to TIMESTAMPTZ,
  _city_id UUID,
...
REVOKE ALL ON FUNCTION public.discover_event_rows_default_v2(
  TIMESTAMPTZ, TIMESTAMPTZ, UUID, BOOLEAN, INT, INT
) FROM PUBLIC;

COMMENT ON FUNCTION public.discover_event_rows_default_v2(
  TIMESTAMPTZ, TIMESTAMPTZ, UUID, BOOLEAN, INT, INT
) IS 'Internal fast path for unfiltered city/date discovery.';
