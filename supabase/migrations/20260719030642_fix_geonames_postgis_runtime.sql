-- Make GeoNames reconciliation independent of the schema that owns PostGIS.
-- Existing city rows already persist numeric latitude/longitude alongside the
-- geography column, so proximity matching can use a schema-neutral haversine
-- calculation while new locations still use the extension schema discovered
-- from pg_catalog.
