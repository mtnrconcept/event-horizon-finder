-- Bound legacy map-pin work before applying the optional filter graph.
-- This follows the serialized-persistence timeout migration already on main.
-- Pre-v2 service-worker clients can still call this signature until their
-- cached application shell is replaced, so the compatibility endpoint must
-- cap database work as well as its serialized response.
...
begin
  ...
  execute $query$
    ...
    fallback_points as materialized (
      ...
      order by fallback.starts_at, fallback.occurrence_id
      limit 6000
    ),
...
comment on function public.discover_map_pins_in_bounds_v1(
  ...
  timestamptz, timestamptz, text[], boolean, text, text[], numeric, numeric,
  boolean, integer, integer, boolean, boolean, boolean, boolean, boolean
) is 'Compatibility endpoint for pre-v2 clients. Bounds candidate work to 12,000 rows and returns at most 4,000 ordered legacy event pins.';
