-- Keep pre-v2 clients responsive when they request a very large geographic and
-- temporal window. Current clients call v2 directly and preserve their full range.
...
begin
  ...
  effective_to := least(
    _to,
    _from + case
      when longitude_span >= 90 or (_north - _south) >= 45 then interval '14 days'
      when longitude_span >= 30 or (_north - _south) >= 15 then interval '30 days'
      when longitude_span >= 8 or (_north - _south) >= 4 then interval '90 days'
...
comment on function public.discover_map_pins_in_bounds_v1(
  ...
  timestamptz, timestamptz, text[], boolean, text, text[], numeric, numeric,
  boolean, integer, integer, boolean, boolean, boolean, boolean, boolean
) is 'Compatibility adapter for pre-v2 clients. Reuses v2 and bounds the temporal horizon according to viewport size to stay below the Data API statement timeout.';
