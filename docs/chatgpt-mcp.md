...
```text
https://event-horizon-finder.vercel.app/mcp
```
...
3. Open **Settings → Plugins**, create a developer-mode app, and enter `https://event-horizon-finder.vercel.app/mcp` as the MCP server URL.
...
```bash
curl --fail-with-body https://event-horizon-finder.vercel.app/mcp \
